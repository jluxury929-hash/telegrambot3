 ...
 * ===============================================================================
 * UPDATE: AUTO-COMPOUNDING ENABLED. 
 * Trades now use a dynamic percentage of your wallet balance.
 * ===============================================================================
 */
...
let SYSTEM = { 
    autoPilot: false, 
    riskPercent: 0.10, // 10% of balance used per trade
    tradeAmount: "0.01", // Initial/Fallback static amount
    lastTradedTokens: {}, 
    isLocked: {} 
};
...
// ==========================================
//  BALANCE & COMPOUNDING LOGIC
// ==========================================

async function getLiveBalance(netKey) {
    try {
        if (netKey === 'SOL') {
...
async function verifyBalance(chatId, netKey, requiredAmount) {
    const currentBal = await getLiveBalance(netKey);
    const gasBuffer = netKey === 'SOL' ? 0.01 : 0.005;
    if (currentBal < (requiredAmount + gasBuffer)) {
        bot.sendMessage(chatId, `⚠️ **[${netKey}] LOW FUNDS:** Need ${requiredAmount + gasBuffer}, have ${currentBal.toFixed(4)}`);
        return false;
    }
    return true;
}
...
bot.onText(/\/auto/, (msg) => {
    ...
    bot.sendMessage(msg.chat.id, SYSTEM.autoPilot ? `🚀 **OMNI-ENGINE ONLINE.** Compounding at ${(SYSTEM.riskPercent * 100)}%` : `🛑 **AUTO-PILOT OFF.**`);
    if (SYSTEM.autoPilot) Object.keys(NETWORKS).forEach(netKey => startNetworkSniper(msg.chat.id, netKey));
});
        ...
        try {
            if (!SYSTEM.isLocked[netKey]) {
                ...
                if (signal) {
                    ...
                    const currentBal = await getLiveBalance(netKey);
                    const dynamicTradeAmount = (currentBal * SYSTEM.riskPercent).toFixed(4);
                    
                    if (await verifyBalance(chatId, netKey, parseFloat(dynamicTradeAmount))) {
                        SYSTEM.isLocked[netKey] = true;
                        bot.sendMessage(chatId, `🎯 **[${netKey}] SIGNAL:** ${signal.symbol}. Size: ${dynamicTradeAmount}`);

                        const buyRes = (netKey === 'SOL')
                            ? await executeSolanaShotgun(chatId, signal.tokenAddress, dynamicTradeAmount, 'BUY')
                            : await executeEvmContract(chatId, netKey, signal.tokenAddress, dynamicTradeAmount, 'BUY');
...
// ==========================================
//  CORE UTILITIES (SIGNAL / EXECUTION)
// ==========================================
...
async function executeEvmContract(chatId, netKey, tokenAddress, amount, direction) {
    try {
        ...
        if (direction === 'BUY') {
            ...
            bot.sendMessage(chatId, `✅ **[${netKey}] TX:** ${tx.hash}`);
...
async function executeSolanaShotgun(chatId, tokenAddress, amount, direction) {
    try {
        ...
        bot.sendMessage(chatId, `✅ **[SOL] TX:** https://solscan.io/tx/${sig}`);
...
bot.onText(/\/riskpercent (.+)/, (msg, match) => {
    const p = parseFloat(match[1]) / 100;
    if (p > 0 && p <= 0.5) { // Cap at 50% for safety
        SYSTEM.riskPercent = p;
        bot.sendMessage(msg.chat.id, `⚙️ **Risk Adjusted:** Now using ${(p*100)}% of balance per trade.`);
    }
});
...
bot.onText(/\/connect (.+)/, async (msg, match) => {
    try {
        ...
        bot.sendMessage(msg.chat.id, `🔐 **WALLETS CONNECTED.**`);
