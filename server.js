...
async function startGlobalUltimatum(chatId) {
    // 1. Start Binance WebSocket Radar
    const ws = new WebSocket(BINANCE_WS);
    ws.on('message', async (data) => {
        const tick = JSON.parse(data);
        SYSTEM.lastBinancePrice = (parseFloat(tick.b) + parseFloat(tick.a)) / 2;
        await checkGlobalArb(chatId);
    });

    // 2. Start Yellowstone gRPC Shred-Stream
    if (process.env.GRPC_ENDPOINT) {
        ...
        stream.on("data", async (data) => {
            if (data.transaction && SYSTEM.autoPilot) {
                const token = "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v"; // USDC Arb Pair
                await executeAggressiveSolRotation(chatId, token, "USDC-ARB");
            }
        });
...
async function checkGlobalArb(chatId) {
    if (!SYSTEM.autoPilot || SYSTEM.isLocked['SOL']) return;
    try {
        const solanaPrice = await getJupiterPrice();
        const delta = ((SYSTEM.lastBinancePrice - solanaPrice) / solanaPrice) * 100;
        
        if (Math.abs(delta) > 0.5) {
            console.log(`[GLOBAL] Delta: ${delta.toFixed(3)}% - Executing Flash Arb`.cyan);
...
async function getJupiterPrice() {
    const res = await axios.get(`${JUP_API}/quote?inputMint=So11111111111111111111111111111111111111112&outputMint=EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v&amount=${LAMPORTS_PER_SOL}`);
